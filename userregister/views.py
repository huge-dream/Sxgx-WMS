...
def register(request, *args, **kwargs):
    if request.method.lower() == 'post':
        ...
        if Users.objects.filter(name='admin', developer=1, is_delete=0).exists():
            user_info = staff.objects.filter(staff_name='admin',
                                 staff_type='Admin').first()
            ...
            if binproperty.objects.filter(openid__iexact='init_data').exists():
                if binproperty.objects.filter(openid__iexact='init_data').count() != 4:
                    ...
                    init_data = [
                        ...
                        binproperty(id=2, openid='init_data', bin_property='Inspection', creater='GreaterWMS'),
            ...
            transaction_code = data.get('openid', '')
            user = User.objects.create_user(username='admin',
                                            password='admin')
            Users.objects.create(user_id=user.id, name='admin',
                                 openid=transaction_code, appid=Md5.md5('admin' + '1'),
                                 t_code=Md5.md5(str(timezone.now())),
                                 developer=1, ip=ip)
            check_code = random.randint(1000, 9999)
            user_id = staff.objects.create(staff_name='admin',
                                 staff_type='Admin',
